...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (i = 0; i < (da->size); i++) {
    new_data[i] = da->data[da->start];
    if(da->start == (da->size-1)) {
      da->start = 0;
    } else {
      da->start++;
    }
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
   else if ((da->start == da->size) || (da->start != 0 && da->size != da->capacity)) {
     da->data[da->size - da->start] = val;
     da->size++;
