...
struct dynarray* dynarray_create() {
  struct dynarray* arr = malloc(sizeof(struct dynarray));
	arr->data = malloc(2*sizeof(void*)); // since each element is a void then data has to be void too
	arr->capacity = 2;
	return arr;
}
...
void dynarray_free(struct dynarray* da) {
  	free(da->data);
    free(da);
	 //why do we have a return function
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  //da->size = da->size+1;
  // if there isn't enough space in the dynamic array:
  int new_Cap = 2* da->capacity;
  if( da->size == da->capacity){
    void **arrTemp = malloc(new_Cap*sizeof(void*)); // will make the capacity bigger
...
void dynarray_remove(struct dynarray* da, int idx) {
  // the reason i would be idx is because it is the specified element in the array that you want to remove
