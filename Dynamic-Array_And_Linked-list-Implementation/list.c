...
void list_free(struct list* list) {
  struct link* start = list->head;
  //struct link* nxtPoint;
  //bool check = false;
  while(start) {
    //nxtPoint = start->next;
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
 struct link* now = list->head;
 struct link* back;
 if(cmp(val,list->head->val) == 0) {
   struct link* temp2 = list->head->next;
   free(list->head);
   list->head = temp2;
   //break;
   return;
...
void list_reverse(struct list* list) {
  struct link* start = list->head;
  struct link* nxtPoint = NULL;
  struct link* back = NULL;
  while(start) {
    nxtPoint = start->next;
